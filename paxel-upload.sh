...
list_projects_grouped() {
  ...
  if [ ${#all_dirs[@]} -eq 0 ] && [ "$has_codex" -eq 0 ] && [ "$has_cursor" -eq 0 ] && [ "$has_opencode" -eq 0 ] && [ "$has_gemini" -eq 0 ]; then
    # Cursor + opencode need host sqlite3 + jq to read; Git Bash on Windows bundles
    # neither, so a Cursor-only user with the tools missing looks empty here (the
    # has_cursor/has_opencode probes above are gated on sqlite3). Point them at the fix
    # instead of a bare "nothing found" (PAXEL: siddharth, Cursor-only on Windows).
    if { [ -d "$CURSOR_DIR" ] || [ -f "$CURSOR_GLOBAL_DB" ] || [ -d "$OPENCODE_DIR" ]; } \
       && { ! command -v sqlite3 >/dev/null 2>&1 || ! command -v jq >/dev/null 2>&1; }; then
      echo "Found Cursor/opencode data but can't read it — sqlite3 and jq are required." >&2
