# TimeSformer hyperparameter ranges (same split as CNN-LSTM)
...
4. **`--aug`**: `strong` (default, matches CNN-LSTM trainers) vs `medium` vs `mild` — if train ≫ val, try **medium/mild**.
...
| `--lr-mult` | `5` | `3` – `8` | Higher multiplier on trunk vs base `--lr` |
