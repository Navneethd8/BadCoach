"""
SkateFormer-B (4-stream skeleton) + ViT image-context late fusion on FineBadminton.

Loads RGB frames and MediaPipe pose from ``pose_cache_mediapipe.pt``.
Warm-start skeleton from pose-only SkateFormer or SkateFormer-B checkpoints.

Example (EC2):

  python backend/pipelines/training/train_skateformer_b.py \\
    --batch-size 4 --resume-skeleton backend/models/badminton_model_skateformer.pth \\
...
from core.skateformer_b import (
    build_skateformer_b_fusion,
    default_skateformer_b_checkpoint_path,
...
def train_skateformer_b(
    ...
    if save_path is None:
        save_path = default_skateformer_b_checkpoint_path(backend_root)
    ...
    mlflow.set_experiment("IsoCourt_Training_SkateFormer_B")
    with mlflow.start_run():
        mlflow.log_params(
            {
                ...
                "lr": lr,
                "seed": seed,
                "script": "train_skateformer_b.py",
                "architecture": "skateformer_b",
                "four_stream": four_stream,
                "embed_dim": embed_dim,
                "num_heads": num_heads,
        ...
        model = build_skateformer_b_fusion(
            task_classes,
            window_size=T,
            embed_dim=embed_dim,
            num_heads=num_heads,
        ...
        if resume_skeleton and os.path.exists(resume_skeleton):
            load_skateformer_b_skeleton_branch(model, resume_skeleton, device=device)
        if resume_checkpoint and os.path.exists(resume_checkpoint):
            ckpt = torch.load(resume_checkpoint, map_location=device, weights_only=False)
            if "skateformer_b" in ckpt:
                model.load_state_dict(ckpt["skateformer_b"], strict=False)
                best_acc = float(ckpt.get("best_acc", 0.0))
                print(
                    f"Loaded SkateFormer-B fusion from {resume_checkpoint} "
        ...
        print(
            f"\nSkateFormer-B+ViT | T={T} four_stream={four_stream} | "
            f"trainable params: {trainable:,} | vit={vit_model_name} "
            f"unfreeze_last={vit_unfreeze_last_n}"
        )
        print(
        ...
        for epoch in range(start_epoch, epochs):
            ...
            if val_acc > best_acc:
                ...
                torch.save(
                    {
                        "skateformer_b": model.state_dict(),
...
def main() -> None:
    p = argparse.ArgumentParser(description="Train SkateFormer-B + ViT late fusion.")
    p.add_argument("--epochs", type=int, default=60)
    p.add_argument("--batch-size", type=int, default=4)
    p.add_argument("--lr", type=float, default=1e-4)
    p.add_argument("--embed-dim", type=int, default=64)
    p.add_argument("--num-heads", type=int, default=16)
    p.add_argument("--no-four-stream", action="store_true", help="3-ch joint-only ablation.")
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Pose-only SkateFormer/SkateFormer-B .pth to warm-start skeleton trunk.",
    )
    p.add_argument(
        "--resume-checkpoint",
        type=str,
        default=None,
        help="Full SkateFormer-B fusion .pth (key skateformer_b).",
    )
