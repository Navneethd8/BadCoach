...
def train_full(
    ...
    device="cpu",
    hidden_size=128,
    save_path="models/badminton_model_full.pth"
):
    ...
    st_labels = torch.tensor(st_labels)
    ...
    # Set num_workers to 0 for stability on macOS (Shared memory manager issue)
    num_workers = 0
    ...
    # This allows the model to adapt to different floors/lighting without "forgetting" how to see objects
    ...
    # Lower LR for backbone to preserve pre-trained features
    # Higher LR for heads to learn the task quickly
    optimizer = optim.Adam([
        {'params': model.cnn.parameters(), 'lr': lr * 0.1}, # 1e-5
        {'params': model.lstm.parameters(), 'lr': lr * 5},  # 5e-4
        {'params': model.heads.parameters(), 'lr': lr * 5}  # 5e-4
    ], weight_decay=1e-4)
    ...
    best_acc = 0.0
    accumulation_steps = 4 # Simulate batch size 16
    ...
    for epoch in range(epochs):
        model.train()
        running_loss = 0.0
        task_correct = {k: 0 for k in task_classes.keys()}
        total = 0
        ...
        for batch_idx, (frames, labels) in enumerate(pbar):
            ...
            for task, logits in outputs.items():
                ...
                task_correct[task] += (predicted == labels[task]).sum().item()
